...
function loadSettings() {
  ...
  if (!raw) return;
  try { state.settings = { ...state.settings, ...JSON.parse(raw) }; } catch (error) { console.warn('設定解析失敗', error); }
}
...
function updateSelectedFile(file) {
  ...
  if (!file) { el.selectedFileLabel.textContent = '拖曳檔案到此，或點擊選取檔案'; el.metaName.textContent = '—'; el.metaSize.textContent = '—'; el.fileInput.value = ''; return; }
...
function cacheTokenResponse(response) {
  ...
  state.authCache = { accessToken: state.accessToken, expiresAt: now + Math.max(60, expiresInSec - 60) * 1000, user: state.user || state.authCache.user || null };
  saveAuthCache();
}
...
function updateAuthUi() {
  ...
  el.authStateSubtext.textContent = loggedIn ? (state.user?.email || '已取得 Google Drive 授權') : '尚未取得 Google 授權';
...
function logout() {
  if (state.accessToken && google?.accounts?.oauth2) {
    google.accounts.oauth2.revoke(state.accessToken, () => {
      state.accessToken = null; state.user = null; clearAuthCache();
      try { if (window.gapi?.client?.setToken) gapi.client.setToken(null); } catch (error) { console.warn('清空 gapi token 失敗', error); }
      updateAuthUi(); state.files = []; renderTableMessage('請先完成設定並登入。'); el.permissionSummary.textContent = '尚未讀取。'; el.folderWritable.textContent = '—'; el.folderDeletable.textContent = '—'; showNotice('已登出 Google 帳號。');
    });
  } else { state.accessToken = null; state.user = null; clearAuthCache(); updateAuthUi(); }
}
...
function renderFiles(files) {
  ...
  if (!state.accessToken) { renderTableMessage('請先完成設定並登入。'); return; }
  if (!finalList.length) { renderTableMessage('目前沒有可顯示的檔案。'); return; }
...
async function uploadFile() {
  ...
  updateUploadProgress(100, '上傳完成，正在重新整理檔案清單…'); showNotice(`檔案「${file.name}」已上傳完成。`, 'success'); updateSelectedFile(null); await refreshFiles();
}
...
async function deleteFile(file) { clearNotice(); if (!window.confirm(`確定要刪除「${file.name}」嗎？`)) return; const params = buildDriveQueryParams(); await googleApiFetch(`https://www.googleapis.com/drive/v3/files/${encodeURIComponent(file.id)}?${params.toString()}`, { method: 'DELETE' }); showNotice(`檔案「${file.name}」已刪除。`, 'success'); await refreshFiles(); }
async function handleTestConnection() { clearNotice(); await initializeGoogleClients(); if (!state.accessToken) { showNotice('請先點「登入 Google」，授權後再測試連線。'); return; } try { await fetchCurrentUser(); } catch (error) { console.warn('測試連線時讀取使用者資訊失敗', error); } await loadFolderSummary(); await refreshFiles(); showNotice('Google Drive 連線測試成功。', 'success'); }
...
function wireDragAndDrop() {
  ...
  drop.addEventListener('drop', (event) => { const file = event.dataTransfer?.files?.[0] || null; if (file) updateSelectedFile(file); });
}
...
function handleSettingsSubmit(event) { event.preventDefault(); clearNotice(); state.settings = getSettingsFromForm(); saveSettings(); syncSettingsForm(); state.initialized = false; showNotice('本機設定已儲存。若剛調整 Client ID / API Key，請重新登入 Google。', 'success'); maybeBootstrapAuth(); }
...
function bindEvents() {
  ...
  el.uploadBtn.addEventListener('click', async () => { try { await uploadFile(); } catch (error) { updateUploadProgress(0, '等待上傳'); showNotice(error.message, 'error'); } });
...
async function maybeBootstrapAuth() {
  ...
  if (restored) {
    try { await bootstrapAfterAuthenticated({ silentNotice: true }); return; } catch (error) { console.warn('使用快取 token 同步資料失敗，改嘗試靜默更新', error); }
  }
  if (state.authCache.user) { state.user = state.authCache.user; updateAuthUi(); }
  const refreshed = await trySilentTokenRefresh();
  if (!refreshed) {
    state.accessToken = null; updateAuthUi(); renderTableMessage('請先完成設定並登入。');
  }
}

async function init() {
  loadSettings(); loadTheme(); loadAuthCache(); syncSettingsForm(); updateAuthUi(); resetUploadProgress(); bindEvents(); wireDragAndDrop(); await registerServiceWorker(); setupInstallPrompt(); setupGoogleLoaderWatchers(); await checkForUpdates({ silent: true }); if (!state.accessToken) renderTableMessage('請先完成設定並登入。'); }
init();
